...
Make sure every tables are connected
Dim_Department(1), Dim_category(1) connect  with Dim_product (Many)
Dim_product connect with fact_table(Many to many)
 
Load the dataset https://www.kaggle.com/datasets/ziya07/high-dimensional-supply-chain-inventory-dataset into power BI. 
click Transform data, Move to Power Query Editor (PQE)

Rename the table as Fact_Supply_chain
Duplicate the Fact_Supply_chain. Replace the table as Dim_Warehouse
Home -> choose columns. select Warehouse_ID, Region
Remove duplicates (home -> Remove rows -> remove duplicates) as Warehouse_id 

Rename the table as Fact_Supply_chain
Duplicate the Fact_Supply_chain. Replace the table as Dim_Supplier
Home -> choose columns. select Supplier_id, Region
Remove duplicates (home -> Remove rows -> remove duplicates) as Supplier_id
  
 Create Relationship dim_Warehouse(1) with Fact_Supply_chain(many)
 Create Relationship dim_Suplier(1) with Fact_Supply_chain(many)
 Dim_date
	Dim_Date =VAR MinDate = MINX( UNION(
            SELECTCOLUMNS(Fact_table, "Date", Fact_table[order date (DateOrders)]),
            SELECTCOLUMNS(Fact_Supply_chain, "Date", Fact_Supply_chain[Date])
        ),
        [Date]    )
...
VAR MaxDate =
    MAXX(
        UNION(
            SELECTCOLUMNS(Fact_table, "Date", Fact_table[shipping date (DateOrders)]),
            SELECTCOLUMNS(Fact_Supply_chain, "Date", Fact_Supply_chain[Date]) ),
       [Date])
RETURN
CALENDAR(MinDate, MaxDate)
Create relationship between fact_supply_chain and dim_Date
   Dim_Date[Date] → Fact_supply_chain[Date] 
Cardinality: one (Dim_date)  to many (Fact table)
Active
